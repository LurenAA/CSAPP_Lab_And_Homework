...
word f_pc = [
	...
	M_icode == IJXX && M_Cnd && M_dstE == RNONE: M_valA;
...
word d_valA = [
	...
	d_srcA == e_dstE && !(E_icode == IJXX && E_ifun == UNCOND): e_valE;    # Forward valE from execute
...
word d_valB = [
	d_srcB == e_dstE && !(E_icode == IJXX && E_ifun == UNCOND): e_valE;    # Forward valE from execute
...
word e_dstE = [
	...
	E_icode == IJXX && E_ifun == UNCOND : UNCOND;
	1 : E_dstE;
];
